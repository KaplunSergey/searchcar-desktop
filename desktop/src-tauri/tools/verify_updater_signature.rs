...
fn main() {
    ...
    let encoded_signature =
        fs::read_to_string(signature_path).unwrap_or_else(|error| fail(error));
    let decoded_signature = STANDARD
        .decode(encoded_signature.trim())
        .unwrap_or_else(|error| fail(error));
    let decoded_signature =
        String::from_utf8(decoded_signature).unwrap_or_else(|error| fail(error));
    let signature = Signature::decode(&decoded_signature).unwrap_or_else(|error| fail(error));
