...
8. Закрытие окна оставляет приложение в tray. Из tray можно открыть окно,
   ...
   При команде `Выйти` диалог зависит от состояния очереди: если поиск сейчас
   выполняется, приложение прямо сообщает, что он будет остановлен, а уже
   найденные машины сохранятся в истории как частичный отчёт; если активного
   поиска нет, показывается короткий вопрос `Вы точно хотите выйти?` без
   предупреждения об отмене несуществующей работы.
...
- удалить лицензию как отдельную необратимую операцию, не смешивая это с
  включением и отключением сайтов-источников;
...
- контекстный диалог полного выхода: при активном scan — понятное предупреждение
  об остановке поиска и сохранении частичного результата, без активного scan —
  только `Вы точно хотите выйти?`;
...
- заменить технические тексты scheduler на пользовательские. Целевой смысл:
  `Поставить автообновление на паузу` — поиск по расписанию временно не
  запускается, но ручной поиск доступен; `После сна или выключения` — пока
  компьютер спит или выключен, поиск не идёт, после возвращения пропущенная
  проверка запускается один раз, а уже найденные данные прерванного поиска
  остаются в истории;
...
Статус: реализована в коде. Готовы локальные first-run без клиентского
пароля, signed allow-list источников, Encar по умолчанию, owner-переключатели,
backend/worker-проверка и миграция существующих pilot-установок. Перед
очередным публичным релизом остаётся только подтвердить этот сценарий свежими
сборками на macOS и Windows.
...
- NSIS `.exe`;
- ad-hoc signed `.dmg`;
- перевести release-сборку Windows в GUI subsystem: запуск SearchCar и
  дочернего production sidecar не должен создавать пустое консольное окно;
- показывать сразу после запуска небольшое окно подготовки SearchCar, пока
  распаковывается onefile sidecar, проверяется локальная база и поднимается
  backend; вместо выдуманного процента использовать spinner и честные этапы
  `Запускаем SearchCar` → `Проверяем локальные данные` → `Открываем приложение`;
- при превышении startup timeout или падении sidecar заменить бесконечную
  загрузку понятной ошибкой с действиями `Повторить`, `Создать отчёт` и `Выйти`;
- закрытие окна подготовки должно корректно останавливать уже запущенные
  дочерние процессы, а закрытие скрытой консоли больше не должно быть способом
...
- в истории запусков — фильтр машин по одному или нескольким проектам рядом с
  фильтром статусов; варианты строятся из проектов выбранного отчёта, включая
  сохранённые названия уже удалённых проектов;
- фильтры проектов и статусов применяются совместно по правилу `И`, по
  умолчанию показываются все проекты, а сброс возвращает полный отчёт; поведение
  одинаково в web и desktop;
- после переименования существующего проекта его текущее имя отображается во
  всех доступных отчётах и вариантах фильтра; историческое имя внутри отчёта
  используется только как fallback для удалённого проекта и не требует
  переписывать сохранённые результаты запусков;
- для каждого нового проекта первый **успешный полный** поиск принудительно
  выполняется как `Точный + Все страницы`: карточки всех найденных машин
  открываются и сохраняются, даже если пользователь выбрал быстрый режим;
- форма создания, API и импорт новых проектов по умолчанию выбирают `Все
  страницы`; после начального полного прохода проект возвращается к сохранённому
  режиму деталей (по умолчанию `Быстрый`), а существующие проекты без явного
  изменения не перенастраиваются;
- отменённый, прерванный сном или завершившийся общей ошибкой проекта первый
  запуск не считается начальным полным проходом: следующая попытка снова
  выполняется как `Точный + Все страницы`; отдельные непрочитанные карточки
  остаются в отчёте, но не заставляют бесконечно повторять полный точный поиск;
- в каждой строке ошибки чтения объявления (например, `INCOMPLETE_PAGE`) выводить
  безопасную ссылку `Открыть объявление` на исходную карточку Encar; она
  открывается системным браузером и позволяет пользователю вручную проверить
  машину, не скрывая технические детали ошибки;
- если URL объявления не был получен или не проходит валидацию, ссылка не
  отображается, а отчёт остаётся доступен с понятной причиной;
...
- экран `Настройки → Диагностика`, выбор периода (последний час, 24 часа, 3 или
  7 дней) и кнопка `Создать отчёт для поддержки` с системным выбором папки;
  если нативный диалог WebView недоступен, использовать защищённое скачивание
  ZIP как fallback;
...
в смешанном отчёте нескольких проектов пользователь может оставить машины
только выбранных проектов и дополнительно сузить их существующим статус-фильтром.
На Windows приложение запускается без консоли; во время реальной подготовки
видно отдельное окно состояния, а ошибка запуска не оставляет ни пустого окна,
ни осиротевшего sidecar-процесса.
