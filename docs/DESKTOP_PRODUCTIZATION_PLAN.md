...
### 8.5 Облачные аккаунты клиентов и trial по аккаунту — отдельная следующая фаза

Текущий pilot правильно выдаёт пробный период устройству: это уже работает и
не требует хранения пароля на сервере. Для коммерческого сценария после
лицензионной админки будет добавлен отдельный клиентский аккаунт:

- регистрация по email и подтверждение адреса;
- безопасный вход, выход, восстановление доступа и ограничение попыток;
- один trial на подтверждённый аккаунт, а не на переустановку приложения;
- лицензия и активационный код принадлежат аккаунту;
- устройство лишь получает ограниченный по времени подписанный lease;
- список устройств, самостоятельный выход с устройства и понятный перенос;
- минимизация персональных данных: email и технические идентификаторы
  лицензии, без проектов, автомобилей, изображений и локальных паролей;
- миграция существующих pilot-лицензий только после явного согласия владельца.

Критерий: пользователь может восстановить доступ на новом устройстве без
ручного вмешательства владельца, а один account-trial нельзя получить повторно
через переустановку или смену устройства.
...
пробуждения выполняется не более одного разрешённого catch-up run. Если ручное
...
### Фаза 8.5. Облачные аккаунты клиентов — 5–8 дней

Задачи:

- регистрация, верификация email и безопасные сессии клиента;
- восстановление доступа и защита от перебора;
- trial, лицензии и активационные коды на уровне аккаунта;
- выдача и обновление device lease после входа;
- self-service список устройств, выход и перенос;
- миграция pilot-лицензий и тесты account/device/reinstall сценариев.

Критерий: trial принадлежит проверенному аккаунту, а не установке приложения;
лицензию можно безопасно восстановить на новом устройстве.
