...
from app.models import Car, CarImage, Project, ProjectCar, ScanRun, User
...
def test_scan_report_exposes_the_current_main_vehicle_image() -> None:
    engine = create_engine("sqlite+pysqlite:///:memory:")
    Base.metadata.create_all(engine)
    with Session(engine, expire_on_commit=False) as db:
        user = User(
        ...
        db.add_all(
            [
                ProjectCar(project_id=project.id, car_id=car.id, tracking_enabled=True),
                CarImage(
                    car_id=car.id,
                    kind="MAIN",
                    payload={"path": "cars/1/main-image.jpg"},
                ),
                run,
            ]
        )
        ...
        assert result["payload"]["report"][0]["image"] == "/storage/cars/1/main-image.jpg"
