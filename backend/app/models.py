...
class SchedulerSetting(Base):
    __tablename__="scheduler_settings"; id: Mapped[int]=mapped_column(primary_key=True); user_id: Mapped[int]=mapped_column(ForeignKey("users.id"), unique=True, index=True); enabled: Mapped[bool]=mapped_column(default=False); paused: Mapped[bool]=mapped_column(default=False); catch_up_enabled: Mapped[bool]=mapped_column(default=True); interval_minutes: Mapped[int]=mapped_column(default=180); next_run_at: Mapped[datetime|None]=mapped_column(UTCDateTime()); last_completed_run_at: Mapped[datetime|None]=mapped_column(UTCDateTime()); created_at: Mapped[datetime]=mapped_column(UTCDateTime(), server_default=func.now()); updated_at: Mapped[datetime]=mapped_column(UTCDateTime(), server_default=func.now(), onupdate=func.now())
