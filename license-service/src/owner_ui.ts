...
<body>
  ...
  (() => {
    ...
    const form = (id, handler, messageId) => $(id).addEventListener('submit', async event => { event.preventDefault(); const button=event.currentTarget.querySelector('button'); button.disabled=true; message(messageId,''); try { const result=await handler(new FormData(event.currentTarget)); event.currentTarget.reset(); await load(); message(messageId,result && result.notice ? result.notice : 'Готово.'); } catch(e) { message(messageId,'Ошибка: '+e.message,true); } finally { button.disabled=false; } });
    ...
    form('customer-form', f => mutation('/v1/owner/customers',{display_name:f.get('display_name'),contact:f.get('contact') || undefined}), 'dashboard-message');
